...
public class MTDDecider implements Decider {
	 ...
	 */
	public MTDDecider(boolean maximizer, int searchTimeSec, int maxdepth, int choice) {
		this(maximizer, searchTimeSec, maxdepth, true);// TODO: change this last true to false to remove mtd
	...
	private int AlphaBetaWithMemory(State state, int alpha, int beta,
		...
		if (depth == 0 || state.getStatus() != Status.Ongoing) {
			int h;
			leafNodes++;
			h = (int) state.heuristic(choice);//chooce heuristic
			int value = color * h;
